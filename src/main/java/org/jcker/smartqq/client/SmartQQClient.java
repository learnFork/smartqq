...
import com.jcker.smartqq.model.*;
...
import org.jcker.smartqq.model.*;
...
public class SmartQQClient implements Closeable {
    ...
    private void login() {
        ...
        System.out.println(userInfo.getNick() + "，欢迎！");
    }
...
/*        Desktop desk = Desktop.getDesktop();
    ...
    private String verifyQRCode() {
        ...
        while (true) {
            ...
            if (result.contains("成功")) {
                ...
                System.out.println("二维码已失效，尝试重新获取二维码");
                getQRCode();
            }
        }
    }
    ...
    private void getPtwebqq(String url) {
        System.out.println("开始获取ptwebqq");

        Response<String> response3 = get(ApiURL.GET_PTWEBQQ, url);
        this.ptwebqq = response3.getCookies().get("ptwebqq").iterator().next().getValue();

        //登录流程4：获取vfwebqq
        System.out.println("开始获取vfwebqq");

        Response<String> response4 = get(ApiURL.GET_VFWEBQQ, ptwebqq);
        this.vfwebqq = getJsonObjectResult(response4).getString("vfwebqq");
    }

    //登录流程5：获取uin和psessionid
    private void getUinAndPsessionid() {
        System.out.println("开始获取uin和psessionid");
    ...
    private void pollMessage(MessageCallback callback) {
        ...
        for (int i = 0; array != null && i < array.size(); i++) {
            ...
            if ("message".equals(type)) {
                ...
                callback.onGroupMessage(message.getJSONObject("value").toJSONString());
...
//        System.out.println("开始发送群消息");
    ...
    private static void checkSendMsgResult(Response<String> response) {
        if (response.getStatusCode() != 200) {
            System.out.println(String.format("发送失败，Http返回码[%d]", response.getStatusCode()));
        }
...
//            System.out.println("发送成功");
            ...
            System.out.println(String.format("发送失败，Api返回码[%d]", json.getInteger("retcode")));
        }
    }
    ...
    private static JSONObject getResponseJson(Response<String> response) {
        ...
        if (retCode == null) {
            ...
            switch (retCode) {
                case 103: {
                    System.out.println("请求失败，Api返回码[103]。你需要进入http://w.web.com，检查是否能正常接收消息。如果可以的话点击[设置]->[退出登录]后查看是否恢复正常");
                    break;
                }
                case 100100: {
